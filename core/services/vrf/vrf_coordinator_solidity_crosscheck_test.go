...
func TestRegisterProvingKey(t *testing.T) {
	...
	assert.Equal(t, coordinator.neil.From, serviceAgreement.VRFOracle,
...
func TestRandomnessRequestLog(t *testing.T) {
	...
	assert.Equal(t, coordinator.consumerContractAddress, log.Sender, "VRFCoordinator logged different requester address from randomness request!")
