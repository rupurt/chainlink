...
func bootstrapORM(t *testing.T) (*orm.ORM, func()) {
	...
	migrationTestDBURL, err := dropAndCreateMigrationTestDB(tc.DatabaseURL())
...
func dropAndCreateMigrationTestDB(databaseURL string) (string, error) {
	parsed, err := url.Parse(databaseURL)
	if err != nil {
		return "", err
	}

	dbname := fmt.Sprintf("%s_migrations", parsed.Path[1:])
